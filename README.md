...
```bash
docker pull moelinx/tinypostgresql
```

Alternatively you can build the image yourself.

```bash
docker build -t m0elnx/tinypostgresql github.com/m0elnx/tinypostgresql
```
...
```bash
docker run --name postgresql -itd --restart always \
  m0elnx/tinypostgresql -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  m0elnx/tinypostgresql -c logging_collector=on
```
...
To upgrade to newer releases:
  ...
  ```bash
  docker pull m0elnx/tinypostgresql:latest
  ```
